...
import eduSkillVector from '../images/home/education-skill/edu-skill-vector.svg'
import { pageData } from "../data/page-data";

const EducationSkills = () => {
  const [educationData, setEductionData] = useState(pageData.educationData);

  useEffect(() => {
    // Optional: load from localStorage if editable later
    const saved = localStorage.getItem("educationData");
    if (saved) {
        setEductionData(JSON.parse(saved));
    }
  }, []);

  return (
    <section>
      <div className="border-t border-museSoftGray overflow-hidden">
              ...
              <div className="grid grid-cols-2 xs:grid-cols-3 gap-5 xl:gap-7 w-full">
                {educationData?.skills?.map((value: any, index: any) => {
                  return (
                    <div
                      key={index}
                      className="p-4 xl:p-6 border border-museSoftGray rounded-lg flex flex-col gap-5 sm:gap-10 items-center justify-between"
                    >

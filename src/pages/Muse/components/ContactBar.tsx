...
const ContactBar = () => {
  const [contactBarData, setContactBarData] = useState(pageData.contactBar);

  useEffect(() => {
    // Optional: load from localStorage if editable later
    const saved = localStorage.getItem("contactBarData");
    if (saved) {
      setContactBarData(JSON.parse(saved));
    }
  }, []);

  return (
    <section>
      <div className="border-t border-museSoftGray">
              ...
              {contactBarData.contactItems.map(
                (value: any, index: number) => (
                  ...
                  >
                    ...
                    <h6 className="text-sm md:text-base xl:text-xl hover:text-musePrimary">
                      {value?.label}
                    </h6>
                  </Link>
                )
